...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "listen error, cause: %s\n", err.Error())
		return
	}
	...
	for {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "accept error, cause: %s\n", err.Error())
			return
		}
		...
		go func() {
			...
			for {
				...
				n, err := conn.Read(buf)
				if err != nil {
					fmt.Fprintf(os.Stderr, "recv err, cause: %s\n", err.Error())
					return
				}
				readBuf := buf[:n]
				...
				if err != nil {
					fmt.Fprintf(os.Stderr, "write err, cause: %s\n", err.Error())
				}
			}
		}()
	}
